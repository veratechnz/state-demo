<template>
  <div class="com-a" v-bind:class="{ pinkbg: goPink }">
    <h1>{{stateMsg}}</h1>
    <button type="button" name="button" @click="changeState">Change State</button>
    <button type="button" name="button" @click="changeState">Go Dark</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    changeState: function () {
      ...
      this.$store.commit('changeTheComponentAMessage', ' This is a real call component message state change from a Component A click event')
    }
  },
  // ***************************************************************
